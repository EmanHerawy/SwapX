...
use risc0_steel::{
    config::ETH_SEPOLIA_CHAIN_SPEC, ethereum::EthEvmEnv, Contract, EvmBlockHeader, EvmInput,
};
...
fn main() -> Result<()> {
    ...
    let mut view_call_input = Vec::new();
    // let assume we want to get user balance in multiple blocks
    for block_number in args.blocks {
        ...
        //total_balance += returns._0;
        view_call_input.push(env.into_input()?);
    }

    /// not clear here what's done ?
    println!("proving...");
    ...
    // let calldata = IProvableTradingHistory::submitProofCall {
